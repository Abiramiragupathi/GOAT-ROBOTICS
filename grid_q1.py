...
OBSTACLE_COUNT = 30
MOVE_INTERVAL = 3  
STEP_DELAY = 0.3    # Faster robot movement
...
YELLOW = (255, 255, 0)
BLUE = (0, 0, 255)
LIGHT_BLUE = (173, 216, 230)  # Highlight color for mouse hover
ORANGE = (255, 165, 0)  # Color for the robot path
...
def draw_grid(grid, robot, end, path, obstacles, start=None, temp_highlight=None):
    ...
    for x, y in path:
        pygame.draw.rect(window, ORANGE, (y * CELL_SIZE, x * CELL_SIZE, CELL_SIZE, CELL_SIZE))
...
def main():
    ...
    while running:
        ...
        # Start navigation immediately after setting start and end
        if start and end and not path:
            path = a_star(grid, start, end)  # Calculate path right after setting points

        # Path recalculation and dynamic grid update
